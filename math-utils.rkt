...
;(define (v+ v1 v2) (map sum (zip v1 v2))) ;vecs represented as matrices
...
(define (recenter center scale)
  (compose (partial s* scale) (partial m+ center)))
...
; change to return gen
(define (get-generator domain-size)
  (let ([result
         (argmax cdr (map-and-show (app power-period _ domain-size)
                                   (range 1 domain-size)))])
    (if (and result (= (- domain-size 1) (cdr result))) (car result) result)))
...
;(define (divides k n) (= 0 (modulo n k)))
;(define (find-factor n) (findf (app divides _ n) (range 1 (inc n))))
...
(define (gcd n k) (if (< n k) (gcd k n) (gcd (modulo n k) k)))
(define (coprime n k) (= 1 (gcd n k)))
(define (all-coprime n) (filter (partial coprime n) (range 1 n)))
